...
figtitle = 'Accuracy with both IMUs vs. with back IMU';
title(figtitle);
xlabel('Training data size');
ylabel('Prediction accuracy (%)');
legend({'front and back measurements', 'Just back measurements'}, 'Location', 'NW');
...
figtitle = 'Log-likehood with both IMUs vs. with back IMU';
title(figtitle);
xlabel('Training data size');
ylabel('Log-Likelihood');
legend({'front and back measurements', 'Just back measurements'}, 'Location', 'SE');
figpath = strcat('C:\School\EEE4022S\Gait Sequence Estimation\Figures\', figtitle);
print(figpath, '-depsc');