function [p_start, A, phi, loglik] = CKNN_hmmGMM(body_part)
    if nargin < 1
        body_part = 'front';  
    end
...
%     chosen_feat_dim = 6; % 0 to select the optimal feature subset
%     [observ_seq, state_seq] = featselect(body_part, chosen_feat_dim);
    [observ_seq, state_seq] = CKNN(body_part);
...
%     [p_start1, A1, phi1, loglik] = ChmmGmm(data, state_num, mixture_num, 'p_start0', p_start, 'A0', A, 'phi0', phi, 'iter_num', iter_num, 'cov_type', 'diag', 'cov_thresh', 1e-4);
    ...
    state_num = 4;
    mixture_num = 2;
    [p_start, A, phi, loglik] = ChmmGmm(data, state_num, mixture_num, 'iter_num', iter_num, 'cov_type', 'diag', 'cov_thresh', 1e-4);
